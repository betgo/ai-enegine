...
- `apps/editor`：Vite React 最小编辑器，支持地图尺寸、路径点、塔位、基地血量、怪物属性、防御塔和波次配置，调用 runtime 预览当前有效 JSON，并支持本地导入/导出 `game.json`。
...
- Inspector 显示 File、Runtime、Map Size、Path Points、Tower Slots、Base、Monsters、Towers、Waves 和 `game.json` 区块。
- 可以编辑地图尺寸、路径点、塔位、基地血量、怪物属性、防御塔和波次；schema 有效时 runtime preview 会重建。
...
`docs/project-plan.md` 第一阶段的阶段 1-10 已完成到本地 MVP 闭环。第二阶段 `Playable Runtime MVP` 已完成最小运行入口。第三阶段 `Editor Gameplay Configuration MVP` 已完成最小创作闭环：`apps/editor` 可以配置地图与玩法，导出的 `game.json` 可在 `apps/player` 导入运行。
